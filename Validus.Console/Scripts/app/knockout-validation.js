	...
	function AttachBindingHandlerValidation(bindingName)
	{
		...
		ko.bindingHandlers[bindingName] = {
			"init": function(element, valueAccessor, bindingsAccessor, viewModel, bindingContext)
			{
				...
				if ($(element).data("val") && ko.isObservable(koObservable)) 
				{
					var attributes = { },
					    validators = { };
					...
					{
						$(e.target).queue("knockout.validate", function(next)
						{
							ko.bindingHandlers[bindingName].validate(e.target, valueAccessor, bindingsAccessor, viewModel, bindingContext);

							next();
						});

						if ($(e.target).queue("knockout.validate").length === 1)
						{
							$(e.target).dequeue("knockout.validate");
						}
					}

					ko.utils.registerEventHandler(element, "blur", ValidateEvent);
					ko.utils.registerEventHandler(element, "change", ValidateEvent);

					$(element).tooltip(
					{
					    container: 'body',
					    template: '<div class="tooltip erroTooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>',
						title: function()
						{
			...
			"update": function(element, valueAccessor, bindingsAccessor, viewModel, bindingContext)
			{
				...
				if (baseBinding.update)
				{
					...
					ko.bindingHandlers[bindingName].validate(element, valueAccessor, bindingsAccessor, viewModel, bindingContext, true);
				}
				
				return base;
			},
			"validate": function(element, valueAccessor, bindingsAccessor, viewModel, bindingContext, isKOUpdate)
			{
				var koObservable = ko.isObservable(valueAccessor()) ? valueAccessor() : valueAccessor().data;
				
				if ($(element).data("val") && ko.isObservable(koObservable))
				{
					var errorTitle = ko.utils.peekObservable(koObservable.error);
					
					// TODO: Find a better way to not validate on update for required validation tests
					if (!ko.utils.peekObservable(koObservable.isValid) && (!isKOUpdate || !/required/gi.test(errorTitle))) 
					{
						$(element).data("originalTitle", errorTitle)
